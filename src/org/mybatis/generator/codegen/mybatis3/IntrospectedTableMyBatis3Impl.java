...
import com.facegarden.mybatis.plugins.Tools;
...
public class IntrospectedTableMyBatis3Impl extends IntrospectedTable {
    ...
    public IntrospectedTableMyBatis3Impl() {
        ...
        managerGenerators = new ArrayList<AbstractJavaGenerator>();
    ...
    public void calculateGenerators(List<String> warnings,
        ...
        //扩展service层
        if (null != context.getYouGouServiceGeneratorConfiguration()) {
        	calculateServiceGenerators(warnings, progressCallback);
        }
        //扩展manager层
        if (null != context.getYouGouManagerGeneratorConfiguration()) {
        	calculateManagerGenerators(warnings, progressCallback);
        }
        //扩展cont
        if (null != context.getYouGouControllerGeneratorConfiguration()) {
    ...
    public List<GeneratedJavaFile> getGeneratedJavaFiles() {
        ...
        for (AbstractJavaGenerator javaGenerator : serviceGenerators) {
            ...
            for (CompilationUnit compilationUnit : compilationUnits) {
                GeneratedJavaFile gjf = new GeneratedJavaFile(compilationUnit,
                        context.getYouGouServiceGeneratorConfiguration()
                                .getTargetProject(),
                                context.getProperty(PropertyRegistry.CONTEXT_JAVA_FILE_ENCODING),
                                context.getJavaFormatter());
                answer.add(gjf);
        ...
        //manager
        for (AbstractJavaGenerator javaGenerator : managerGenerators) {
            List<CompilationUnit> compilationUnits = javaGenerator
                    .getCompilationUnits();
            for (CompilationUnit compilationUnit : compilationUnits) {
                GeneratedJavaFile gjf = new GeneratedJavaFile(compilationUnit,
                        context.getYouGouManagerGeneratorConfiguration()
                                .getTargetProject(),
                                context.getProperty(PropertyRegistry.CONTEXT_JAVA_FILE_ENCODING),
                                context.getJavaFormatter());
                answer.add(gjf);
        ...
        for (AbstractJavaGenerator javaGenerator : controllerGenerators) {
            ...
            for (CompilationUnit compilationUnit : compilationUnits) {
                GeneratedJavaFile gjf = new GeneratedJavaFile(compilationUnit,
                        context.getYouGouControllerGeneratorConfiguration()
                                .getTargetProject(),
                                context.getProperty(PropertyRegistry.CONTEXT_JAVA_FILE_ENCODING),
                                context.getJavaFormatter());
                answer.add(gjf);
	...
	protected void calculateManagerGenerators(List<String> warnings,
			ProgressCallback progressCallback) {
		AbstractJavaGenerator javaGenerator = null; //TODO 
		initializeAbstractGenerator(javaGenerator, warnings, progressCallback);
		this.managerGenerators.add(javaGenerator);
	}

	protected void calculateControllerGenerators(List<String> warnings,
			ProgressCallback progressCallback) {
		AbstractJavaGenerator javaGenerator = null; //TODO
		initializeAbstractGenerator(javaGenerator, warnings, progressCallback);
