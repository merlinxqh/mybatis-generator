...
public class BorderLayout extends Layout {
	...
	protected void layout(Composite composite, boolean flushCache) {
		...
		if (this.controls[0] != null) {
			this.controls[0].setBounds(clientArea.x, clientArea.y,
					clientArea.width, this.sizes[0].y);
		}
		if (this.controls[1] != null) {
			this.controls[1].setBounds(clientArea.x, clientArea.y
					+ clientArea.height - this.sizes[1].y, clientArea.width,
					this.sizes[1].y);
		}
		if (this.controls[4] != null) {
			this.controls[4].setBounds(clientArea.x, clientArea.y
					+ this.sizes[0].y, this.sizes[4].x, clientArea.height
					- this.sizes[0].y - this.sizes[1].y);
		}
		if (this.controls[3] != null) {
			this.controls[3].setBounds(clientArea.x + clientArea.width
					- this.sizes[3].x, clientArea.y + this.sizes[0].y,
					this.sizes[3].x, clientArea.height - this.sizes[0].y
							- this.sizes[1].y);
		}
		if (this.controls[2] != null) {
			this.controls[2].setBounds(clientArea.x + this.sizes[4].x,
					clientArea.y + this.sizes[0].y, clientArea.width
							- this.sizes[4].x - this.sizes[3].x,
	...
	private void refreshSizes(Control[] children) {
		...
		this.width = Math.max(this.width, this.sizes[4].x + this.sizes[2].x
				+ this.sizes[3].x);
		this.width = Math.max(this.width, this.sizes[1].x);
		this.height = (Math.max(Math.max(this.sizes[4].y, this.sizes[3].y),
