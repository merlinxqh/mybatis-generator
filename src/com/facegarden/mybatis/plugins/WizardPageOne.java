...
public class WizardPageOne extends WizardPage {
	...
	public void createControl(Composite parent) {
		...
		if ((this.tableMap != null) && (this.tableMap.size() > 0)) {
			this.combo.setItems((String[]) this.tableMap.keySet().toArray(
					new String[this.tableMap.keySet().size()]));
		}
		...
		this.text.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent arg0) {
				...
				String key = WizardPageOne.this.combo.getText();
				if (key.equals("")) {
					MessageDialog.openConfirm(WizardPageOne.this.getShell(),
							"提交", "请选择对应的表空间");
					return;
				}
				...
				for (String k : tableMap.get(key)) {
					if (k.toLowerCase().indexOf(
							WizardPageOne.this.text.getText().toLowerCase()) != -1) {
						newTableList.add(k);
					}
				}
			...
			public void widgetSelected(SelectionEvent e) {
				...
				for (int i = 0; i < WizardPageOne.this.table.getItems().length; i++) {
					if (WizardPageOne.this.table.getItems()[i].getChecked()) {
						WizardPageOne.this.tableList
								.add(WizardPageOne.this.combo.getText()
										+ "."
										+ WizardPageOne.this.table.getItems()[i]
												.getText());
					}
				}
